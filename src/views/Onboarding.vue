          ...
          <v-stepper class="elevation-0" v-model="e1">
            <v-stepper-header>
              ...
              <v-stepper-step step="3" :complete="finished" color="accent">
                <span class="accent--text">Payment Info</span>
              </v-stepper-step>
            </v-stepper-header>
            <v-divider></v-divider>
            <v-stepper-items>
              ...
              <v-stepper-content step="2">
                <v-card class="mb-12" height="400px">
                  ...
                  <v-row :align="alignment" :justify="justify">
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field label="Occupation" v-model="occupation" filled color="primary"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
              ...
              <v-stepper-content step="3">
                <v-card class="mb-12" height="400px">
                  <v-row :align="alignment" :justify="justify">
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field :rules="rules" type="text" prefix="Ksh" label="Registration Fee" v-model="regFee"
                                    filled color="primary"></v-text-field>
                    </v-col>
                    <v-col cols="12" sm="6" md="4">
                      <v-text-field :rules="rules" type="text" prefix="Ksh" label="Share Capital" v-model="shareCapital"
                                    filled color="primary"></v-text-field>
                    </v-col>
                  </v-row>
                </v-card>

                <v-btn color="accent" @click="PayForReg()">Continue</v-btn>
...
<script lang="ts">
  ...
  export default class Onboarding extends Vue {
    ...
    CreateMember() {
      ...
      this.$store.dispatch("registerUser", this.data).then(resp => {
        this.e1 = 3;
      }).catch(err => {
        console.log(err)
      })
    ...
    PayForReg() {
      console.log(this.regFee)
      if (this.regFee != '' && this.paymentData != '') {
        this.paymentData = {
          "header": {
          ...
          "lines": [
            {
              ...
              "amount": parseInt(this.regFee.trim())
            },
            {
              "transaction_type": "shares",
              "amount": parseInt(this.shareCapital.trim())
            }
          ],
          "phone": 254727292911
        }
        this.$store.dispatch("makePayment", this.paymentData).then(resp => {
    ...
    mounted() {
      this.$store.dispatch("fetchUser").then(resp => {
        if (resp.data != false) {
          this.e1 = 3;
        }
      }).catch(err => {
